 ...
 */
const shopifyHost = "https://nhkstore1412.myshopify.com";
const themeId = "80549281876";
const apiKey = "5b510e5f81a8b509fc7d33db49fc2284";
const pwd = "8d10151b88dbc69612639ee05ec7f9f2";
...
async function productionStyles(_minify) {
  ...
  for (let i = 0; i < allTasks.length; i++) {
    let task = allTasks[i];
    if (task.status === "success" && "filePath" in task) {
    ...
    rp({
      ...
      .then((res) => {
        ...
        let match = newLayout.match(
          /\<script.+(?:'|")theme.js(?:'|")\s\|\sasset_url(?:(?!\<\/script\>).)+\<\/script\>/gm
        );
        let appendLinks = `<script src="{{ 'theme.js' | asset_url }}"></script>\n<script src="{{ 'custom.js' | asset_url }}"></script>`;
  ...
  return (async () => {
    ...
    if (matchVars) {
      ...
      let writeTask = await new Promise((resolve, reject) => {
        fs.writeFile("app/styles/variables.scss", newVars, (err) => {
...
serve = series(startAppServer, parallel(styles, scripts, stylesToMuchCss));
